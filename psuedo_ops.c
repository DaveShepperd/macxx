...
{
    int algn;
    if ((algn = current_section->seg_salign) == 0) return;
    if (sayerr == 0)
    {
        if (algn < size)
        {
    ...
    else
    {
        int i;
        i = macxx_min_dalign;
        if (i < current_section->seg_dalign) i = current_section->seg_dalign;
        if (algn > size) algn = size; /* minimize to size */
        if (algn > i) algn = i;       /* minimize to max of dalign/min */
    }
...
static int if_dfndf_exprs(DFNDF_sense sense,int flag)
{
    ...
    while ((toktyp=get_token()) != EOL)
    {
        ...
        if (flag == 0) break; /* quit if to get only 1 term */
        if ((cttbl[(int)*inp_ptr]&(CT_COM|CT_SMC|CT_EOL)) != 0) break;
    }
    if (termcnt != 1)
    {
        if (termcnt < 1)
...
static unsigned long op_ifcondit(char *condition)
{
	...
	dfcond = DFNDF_FALSE;
    if (tcon == CCN_DF) dfcond = DFNDF_DF;
    if (tcon == CCN_NDF) dfcond = DFNDF_NDF;
	if (tcon == CCN_EXIST) dfcond = DFNDF_EXIST;
	if (tcon == CCN_NEXIST) dfcond = DFNDF_NEXIST;
    ...
    case CCN_NB: {
            ...
            l1 = inp_ptr - arg1;
            if (l1 == 0) j = 0;
            if (tcon == CCN_NB) j ^= 0x80000000L;
            if (*inp_ptr == macro_arg_close) ++inp_ptr;    /* eat the character */
            break;
        }
    case CCN_EQ:
        if (ans == 0) j = 0;
...
int op_maum(void)
{
    if (macxx_mau == 8) return op_byte_with_mask(1);
    if (macxx_mau == 16) return op_word_with_mask(1);
    return op_long();
}
...
int op_2maum(void)
{
    if (macxx_mau == 8) return op_word_with_mask(1);
    if (macxx_mau == 16) return op_long();
    return bad_mau();
}
        ...
        if (*inp_ptr == '=')
        {
            ...
            if (length_names[tt].name != 0)
            {
                if (slen < length_names[tt].min || slen > length_names[tt].max)
                {
                    ...
                    bad_token(tp,emsg);
                    if (slen < length_names[tt].min) slen = length_names[tt].min;
                    if (slen > length_names[tt].max) slen = length_names[tt].max;
                }
            }
            comma_expected = 1;
        }
...
int op_include(void)
{
    ...
    while ((cttbl[(int)*inp_ptr]&(CT_EOL|CT_SMC|CT_WS)) == 0) ++inp_ptr;
    ...
    *(fn_pool+tfnd->r_length) = 0;
    ...
    if (tfnd->fn_nam != 0) tfnd->fn_buff = tfnd->fn_nam->full_name;
    if (rms_err || (tfnd->fn_file = fopen(tfnd->fn_buff,"r")) == 0)
    {
        ...
        if (tfnd->fn_file != 0) fclose(tfnd->fn_file);
    }
    else
    {
        ...
        if (options[QUAL_DEBUG])
        {
            ...
            if (fn_pool_size < 26) (void)get_fn_pool();
                      ...
                      int flags,int salign,int dalign,unsigned long maxlen)
{
    ...
    if (flags != 0)
    {
        ...
        if ((flags&PS_ABS) != 0)
        {
            ...
            {
                new_seg->flg_abs = 1;
                if ((flags&(PS_CON|PS_OVR)) == 0) flags |= PS_OVR;
            }
        }
        if ((flags&PS_REL) != 0)
        {
...
SEG_struct *find_segment(char *name,SEG_struct **segl,int segi)
{
    ...
    {
        new_seg = *(segl+i);
        if (strcmp(name, new_seg->seg_string) == 0) break;
        new_seg = 0;
    }
    return new_seg;
}
...
static SEG_struct *get_segname(char *alt_name, int *new )
{
    ...
    SS_struct *sym_ptr;
    *new = 0;
    if (get_userssegname(alt_name) == 0) return 0;
                ...
                if (psect_table[tt].name == 0)
                {
                    ...
                    else
                    {
                        if (*inp_ptr == '=')
                        {
                            ...
                            {
                                exprs(0,&EXP0);
                                if ((segdat&PS_DALGN) != 0) dalign = EXP0SP->expr_value;
                                if ((segdat&PS_SALGN) != 0) salign = EXP0SP->expr_value;
                                if ((segdat&PS_MAXLN) != 0) maxlen = EXP0SP->expr_value;
                            }
                        }
                        flags |= segdat;
                        break;
...
{
    int flags = 0;
    if (new_one != 0) flags = PS_OVR|PS_RW|PS_ABS;
...
int op_csect(void)
{
    ...
    if (new_seg == 0) return 1;
    if (new_one != 0) flags = PS_OVR|PS_RW;
    op_segcomm(0,new_one,new_seg,flags,0,0,0l);
    return 1;
}
...
int dump_subsects(void)
{
    ...
    for (i=0;i<subseg_list_index;++i)
    {
        ...
        if (new_seg->rel_offset != 0) continue;
        new_seg->seg_string = " ";        /* make it unreachable */
...
int op_escape(void)
{
    ...
    while (1)
    {
        ...
        {
            if (strncmp(token_pool,esc->desc,(int)token_value) == 0) break;
        }
...
int op_test(void)
{
    ...
    {
        while (cttbl[(int)*inp_ptr] != CT_EOL) ++inp_ptr;
    }
...
void op_purgedefines(struct str_sub *sub)
{
	...
	for ( siz = 0; siz < 64; ++siz, ++sub )
    {
        ...
        if (sub->name != 0) MEM_free(sub->name);
        if (sub->value != 0) MEM_free(sub->value);
    }
...
int op_define(void)
{
    ...
    if ((cttbl[(int)*inp_ptr]&(CT_EOL|CT_SMC)) == 0)
    {
        ...
        s = inp_ptr;
        while ((cttbl[(int)*s]&(CT_SMC|CT_EOL)) == 0) ++s;
        --s;
        while ((cttbl[(int)*s]&CT_WS) != 0) --s;
        ++s;
        tt = s-inp_ptr;
...
int op_undefine(void)
{
    ...
    if (string_macros != 0)
    {
        ...
        {
            tt = get_token();
            if (tt != TOKEN_strng && tt != EOL) continue;
            if (tt == EOL)
            {
            ...
            if (strncmp(token_pool,".UNDEFINE",max_opcode_length) != 0) continue;
            break;
        }
    }
    if ((cttbl[(int)*inp_ptr]&(CT_EOL|CT_SMC)) == 0)
