...
int ust_init( void )
{
    if (image_name != 0)
    {
        ...
        if (s != 0 && strncmp(s,"816",3) == 0) options[QUAL_P816] = 1;
    }
    ...
    default: {
            ...
            { /* if mostech format */
                ...
                {     /* item started with a '(' */
                    ...
                    if (*inp_ptr == tst)
                    {
                        ...
                        {
                            if (amflag&OPENBKT) return NND_NUM; /* plain direct long indirect [d] or [a] */
                    ...
                    {             /* -- ended with ']' or ')' */
                        ...
                        {    /* perhaps a (nn,X) format */
                                ...
                                (reg == 'X' || reg == 'S'))
                            {
                                ...
                                if (reg == 'X') return(opc->op_amode&NAX) ? NAX_NUM : NX_NUM;
                                amflag |= GOTSTK;
                            }
                            else
                            {
                ...
                {            /* -+ if opened with a '(' */
                    ...
                    {   /* stop on a comma? */
                        ...
                        if (token_type == TOKEN_strng && token_value == 1)
                        {
                            switch (xy)
                            {
                            case 'X': return X_NUM;  /* syntax is nn,X */
                            case 'Y': return Y_NUM;  /* syntax is nn,Y */
                            case 'S': return DS_NUM; /* syntax is nn,S */
                            }
                        }
            ...
            {               /* -+ if MOS format */
                if (token_type == TOKEN_strng && *inp_ptr == ',')
                {
                    ...
                    if (token_value == 1)
                    {
                        if (c == 'I') amdcdnum = I_NUM;
                        else if (c == 'Z') amdcdnum = Z_NUM;
                        else if (c == 'A') amdcdnum = A_NUM;
                    ...
                    else
                    {
                        if (token_value == 2)
                        {
                            ...
                            if (c == 'Z')
                            {
                                if (c1 == 'X') amdcdnum = ZX_NUM;
                                else if (c1 == 'Y') amdcdnum = ZY_NUM;
                            }
                            else if (c == 'A')
                            {
                ...
                if ((amflag & MNBI) == 0)
                {
                    if (*inp_ptr == open_operand)
                    {
                        ...
                        {
                            ++inp_ptr;     /* eat closing ) */
                            if (amflag&OPENAT) return(xy == 'X') ? NX_NUM : NY_NUM;
                            return(xy == 'X') ? X_NUM : Y_NUM;
                        }
                        if (amflag&OPENAT) bad_token(am_ptr,"Illegal index syntax. Expected @n(X) or @n(Y)");
...
static int make_dpage_ref(EXP_stk *estk)
{
    ...
    if (dp == 0) return 0;   /* nothing to do */
...
static int make_bank_ref(EXP_stk *estk)
{
    ...
    EXPR_struct *src,*dst;
    bp = *(bank+bank_stack);
    if (bp == 0) return 0;   /* nothing to do */
    dst = estk->stack+estk->ptr;
    bpstk = bp->exptr;
    src = bpstk->stack;
...
static int check_4_dpage(EXP_stk *estk)
{
    ...
    expr = estk->stack;
    dp = *(dpage+dpage_stack);
    if (estk->base_page_reference) return 1; /* base page reference */
    if (dp == 0)
    {
        if ((estk->ptr == 1) &&           /* or expression is < 256 */
            (expr->expr_code == EXPR_VALUE) &&
            ((expr->expr_value & -256) == 0) &&
            (estk->forward_reference == 0)) return 1;
    }
    else
    {
        int i;
        ...
        if (i > tmp_expr_size)
        {
            if (tmp_expr != 0) MEM_free((char *)tmp_expr);
...
static int check_am(Opcode *opc, AModes amdcdnum, AModes forced_am_num)
{
    ...
    if (options[QUAL_P816])
    {
        ...
        if (amdcdnum <= UNDEF_NUM) tp = test;
        else if (amdcdnum == X_NUM) tp = test+1;
        else if (amdcdnum == Y_NUM) tp = test+2;
        else if (amdcdnum == N_NUM) tp = test+3;
        else tp = 0;
        if (tp != 0) do
            {
...
static int compute_opcode(Opcode *opc,int amdcdnum)
{
    ...
    if (options[QUAL_P816])
    {
        ...
        {
            if (amdcd&(A+NAX)) make_pbr_ref(&EXP1);
...
void do_opcode(Opcode *opc)
{
    ...
    case OPCL01: {
            int ct;
            if ((opc->op_amode&I) != 0 && *inp_ptr == '#') while (++inp_ptr,isspace(*inp_ptr));
            ct = get_token();
            if (ct == EOL)
            {
...
int op_bank( void )
{
    ...
    if (typ == EOL)
    {
        ...
        {
            if (bank_stack < 2 ||
                *(bank+bank_stack) != *(bank+bank_stack-1)) MEM_free((char *)bank[bank_stack]);
            *(bank+bank_stack) = 0;
            --bank_stack;
        }
        return 2;
    ...
    { /* are we supposed to save the old bank? */
        ...
        if (typ == TOKEN_strng)
        {
            ...
            {
                if (i > 0) bad_token((char *)0,"Expression ignored on bank RESTORE");
                goto pop_bank;
            }
            else
            {
...
int op_dpage( void )
{
    ...
    if (typ == EOL)
    {
        ...
        {
            if (dpage_stack < 2 ||
                *dpage+dpage_stack != *dpage+(dpage_stack-1)) MEM_free((char *)dpage[dpage_stack]);
            *(dpage+dpage_stack) = 0;
            --dpage_stack;
        }
        return 2;
    ...
    { /* are we supposed to save the old dpage? */
        ...
        if (typ == TOKEN_strng)
        {
            ...
            {
                if (i > 0) bad_token((char *)0,"Expression ignored on dpage RESTORE");
                goto pop_dpage;
            }
            else
            {
        ...
        else if (typ == EOL) goto pop_dpage;
    }
    if (i > 0)
    {         /* if 1 or more terms... */
        EXP_stk *exp_ptr;
