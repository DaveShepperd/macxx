...
static void showBadExpression(const EXP_stk *topLevel)
{
	...
	{
		int32_t val = top_exp->expr_value;
		...
		case EXPR_SEG:
			...
			if ( val != 0 )
				len += snprintf(emsg + len, sizeof(emsg) - len, " %%%d %d +", top_exp->expr_seg->seg_ident, val);
			else
		...
		case EXPR_VALUE:
			len += snprintf(emsg + len, sizeof(emsg) - len, " %d", val);
			break;
		case EXPR_OPER:
...
static char *outexp_skip(char *s)
{
	while ( *s++ )
		;
	--s;
...
static char *outexp_ol_fin(char *s, char lb, EXPR_struct *exp)
{
	sprintf(s, " %c %%%d", lb, exp->expr_seg->seg_ident);
	return outexp_skip(s);
}
	...
	*s = 0;          /* null terminate the dst string */
	for (; k > 0; --k, ++exp )
	{
		...
		case EXPR_SEG:
			exp->expr_value += exp->expr_seg->rel_offset;
			if ( exp->expr_value != 0 )
				sprintf(s, " %%%d %d +", exp->expr_seg->seg_ident, exp->expr_value);
			else
...
{
	EXPR_struct *exp;
	uint32_t taddr, laddr, address;       /* address of end of txt + 1 */
	char *s;
	eps->tag = 0;            /* make sure there's no tag */
...
/*	First, predict where the string will end    */
	...
	for ( count = 0, msk = ~0; msk & num; msk <<= 4 )
		++count;
	if ( count == 0 )
		++count;
	vp = &where[count + 1];
...
*/
static int formsym(char *name, int type, uint32_t address, char *where)
{
	int i;
	char *np, c;
	namcs = type;
 ...
 */
	...
	len = formsym(sym_ptr->ss_string, 1, (uint32_t)sym_ptr->ss_value, symp);
...
void outsym_def(SS_struct *sym_ptr, int mode)
{
	...
	case OUTPUT_HEX:
		{    /* absolute output (tekhex/non-relative) */
			outsym(sym_ptr, mode);
			return;        /* easy out */
		}
	case OUTPUT_OL:
		{     /* relative output (.OL format) */
...
/* printf("Doing %d term expression defining \"%s\"\n",sym_ptr->ss_exprs->ptr,sym_ptr->ss_string); */
	...
	case OUTPUT_OBJ:
		{    /* vlda relative */
			...
			if ( sym_ptr->flg_defined )
				flg |= VSYM_DEF; /* symbols may be defined */
			if ( sym_ptr->flg_exprs )
				flg |= VSYM_EXP;
			if ( sym_ptr->flg_abs )
				flg |= VSYM_ABS; /* may-be abs */
			if ( !sym_ptr->flg_global )
				flg |= VSYM_LCL; /* may be local */
...
/*	 outsym(seg_ptr,output_mode); */
	...
	case OUTPUT_OL:
		{     /* relative output (.OL format) */
			...
			if ( seg_ptr->flg_data )
				*s++ = 'd';
			if ( seg_ptr->flg_literal )
				*s++ = 'l';
			if ( seg_ptr->flg_noout )
				*s++ = 'o';
			if ( seg_ptr->flg_ro )
				*s++ = 'r';
	...
	case OUTPUT_OBJ:
		{        /* vlda relative */
			...
			if ( seg_ptr->flg_based )
				flg |= VSEG_BASED; /* may be based */
			if ( seg_ptr->flg_zero )
				flg |= VSEG_BPAGE; /* may be bsect */
			if ( seg_ptr->flg_abs | seg_ptr->flg_based )
				flg |= VSEG_ABS; /* may-be abs */
			if ( seg_ptr->flg_ovr )
				flg |= VSEG_OVR; /* may be overlaid */
			if ( seg_ptr->flg_data )
				flg |= VSEG_DATA; /* segment is data/code */
			if ( seg_ptr->flg_ro )
				flg |= VSEG_RO; /* segment is read/only */
			if ( seg_ptr->flg_reference )
				flg |= VSEG_REFERENCE; /* segment is referenced */
			if ( seg_ptr->flg_literal )
				flg |= VSEG_LITERAL; /* segment is a literal pool */
