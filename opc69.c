...
static void do_branch(Opcode *opc)
{
    ...
    if( (opc->op_amode&SPL)&&(!(opc->op_amode&SP10)) ) offset = 3;
    get_token();
    if (exprs(1,&EXP1) < 1)
...
/* give error here */
            ...
            {  /* (r) or n(r) or @(r) or @n(r) or @nnnn */
                ...
                if( j )
                {
                    ...
                    {
                        if (exprs(1,&EXP2) < 1) return -1;   /* test operand */
                        k = EXP2SP->expr_value;
            ...
            /*          EXP2.ptr = 1;             * third stack has operand (1 element) */
                ...
                if (exprs(1,&EXP1) < 1) break;   /* test operand */
...
/*    EXPR_struct *exp_ptr; */
    ...
    EXP0SP->expr_code = EXPR_VALUE;  /* set the opcode expression *//* expression component is an absolute value */
